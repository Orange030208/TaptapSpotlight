return {
    definitions = {
        {
            id = "prism_dash",
            name = "折光突进",
            description = "完美招架后，0.5 秒内按方向键穿刺前冲，对路径敌人造成伤害。",
            shortDescription = "完美招架后，0.5 秒内\n按方向键穿刺前冲。",
            color = { 104, 232, 255 },
            iconKind = "dash",
            iconPath = "image/ui/crystals/prism_dash.png",
            maxStacks = 1,
        },
        {
            id = "mirror_split",
            name = "三棱分光",
            description = "完美反射的投射物会分裂为三枚晶棱，覆盖更大的扇形区域。",
            shortDescription = "完美反射会裂成\n三枚晶棱弹幕。",
            color = { 247, 171, 255 },
            iconKind = "split",
            iconPath = "image/ui/crystals/mirror_split.png",
            maxStacks = 1,
        },
        {
            id = "thunder_chime",
            name = "雷链谐振",
            description = "每两次完美招架，召下一段自动锁定附近敌人的连锁闪电。",
            shortDescription = "每两次完美招架\n落下连锁闪电。",
            color = { 255, 224, 104 },
            iconKind = "lightning",
            iconPath = "image/ui/crystals/thunder_chime.png",
            maxStacks = 1,
        },
        {
            id = "orbit_shards",
            name = "星轨碎片",
            description = "每次完美招架生成一枚环绕碎晶，最多三枚。每枚持续 8 秒，可抵挡一次攻击。",
            shortDescription = "完美招架生成卫星，\n8 秒内抵挡一次攻击。",
            color = { 150, 135, 255 },
            iconKind = "orbit",
            iconPath = "image/ui/crystals/orbit_shards.png",
            maxStacks = 1,
        },
        {
            id = "nova_core",
            name = "超新星核",
            description = "进入超载时爆开一圈晶体冲击波，对近身敌人造成范围伤害。",
            shortDescription = "进入超载时爆开\n晶体冲击波。",
            color = { 255, 117, 152 },
            iconKind = "nova",
            iconPath = "image/ui/crystals/nova_core.png",
            maxStacks = 1,
        },
        {
            id = "time_heart",
            name = "时隙之心",
            description = "本局首次受到致命伤时保留 1 点生命、清除弹幕，并短暂无敌。",
            shortDescription = "首次致命伤时保命，\n清除全场弹幕。",
            color = { 111, 242, 192 },
            iconKind = "time",
            iconPath = "image/ui/crystals/time_heart.png",
            maxStacks = 1,
        },
        {
            id = "crystal_lattice",
            name = "结晶界碑",
            description = "完美招架会留下晶体锚点。两枚锚点之间形成晶线，切断敌弹并伤害首次穿越的敌人。",
            shortDescription = "完美招架留下锚点，\n两点连成切断敌弹的晶线。",
            color = { 104, 232, 255 },
            iconKind = "dash",
            iconPath = "image/ui/crystals/vine_crystal.png",
            maxStacks = 1,
        },
        {
            id = "mirror_gate",
            name = "镜界折返",
            description = "反射弹首次击杀敌人时，在击杀点留下镜门。敌方弹幕穿过镜门会被折返为你的攻击。",
            shortDescription = "反射击杀留下镜门，\n敌弹穿门后折返。",
            color = { 247, 171, 255 },
            iconKind = "split",
            iconPath = "image/ui/crystals/ripple_crystal.png",
            maxStacks = 1,
        },
        {
            id = "rift_shift",
            name = "裂隙换位",
            description = "连续两次完美招架生成残像锚点。下次受击免伤并换位回锚点，在原地引爆晶核。",
            shortDescription = "两次完美招架存残像，\n下次受击换位爆裂。",
            color = { 255, 117, 152 },
            iconKind = "nova",
            iconPath = "image/ui/crystals/frost_crystal.png",
            maxStacks = 1,
        },
    },

    dash = {
        window = 0.5,
        duration = 0.16,
        speed = 2.8,
        damage = 1.0,
        radius = 0.075,
    },
    split = {
        angle = math.rad(19),
        damageMultiplier = 0.78,
        speedMultiplier = 0.92,
    },
    lightning = {
        requiredPerfects = 2,
        damage = 1.0,
        targetCount = 3,
        range = 0.46,
    },
    orbit = {
        maxShards = 3,
        radius = 0.105,
        shardRadius = 0.032,
        duration = 8.0,
        fadeDuration = 1.5,
        guardDamage = 0.1,
        reflectionSpeedMultiplier = 1.15,
    },
    nova = {
        radius = 0.28,
        damage = 1.25,
    },
    timeHeart = {
        invulnerability = 1.0,
    },
    lattice = {
        maxAnchors = 2,
        duration = 5.0,
        lineRadius = 0.016,
        enemyDamage = 0.9,
    },
    mirrorGate = {
        duration = 4.0,
        radius = 0.062,
        damage = 0.75,
        speedMultiplier = 1.12,
    },
    riftShift = {
        requiredPerfects = 2,
        invulnerability = 0.35,
        novaRadius = 0.19,
        novaDamage = 1.1,
    },
}
