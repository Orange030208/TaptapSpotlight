...
本目录包含 20 个已经接入玩法事件的成品音效。文件名是稳定的事件接口，替换时必须保留
同名 `.ogg` 文件，Lua 中的音量、随机音高和触发时机无需修改。

本批素材的方向是近距离金属碰撞、石质空间反射、压低的重击与克制的魔法残响；不使用
8-bit、界面哔声或科幻激光作为核心音色。每个文件都由 CC0 实录素材分层、均衡、短混响
和响度控制后导出。

## 技术规格

- Ogg Vorbis，48 kHz，立体声
- 起音控制在 5 ms 内，适合招架与命中反馈
- 成品峰值低于 -1.5 dBFS，为游戏中的并发音效预留余量
...
| `battle_start.ogg` | 敌人开始行动 | 实录刀刃相击与厚重金属板撞击 |
